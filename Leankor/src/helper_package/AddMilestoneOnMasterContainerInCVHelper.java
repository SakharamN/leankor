	  ...
	  public void ClickOn_ClonemilestoneByStartDate()
	  {
		  ...
		  //  System.out.println("startdate from calendarpopup is : "+pickstartdatefromcalendarpopup);
			...
			// System.out.println("All date element length on clonestartdate popup :"+getAlldateElementLength);
				...
				{                                                                   
					String classAttributeValueOfTdElement = findElement(By.xpath("//table[contains(@class,'x-datepicker-inner x-focus x-datepicker-focus x-datepicker-default-focus')]//td[contains(@class,'x-datepicker-selected')]/parent::tr/child::td["+i+"]")).getAttribute("class");
					...
					if(picksatrtdatefromCalendarpopupAttributeValue.contains(classAttributeValueOfTdElement) && i<=4)
					{
						...
						try {Thread.sleep(2000);}catch(InterruptedException ie){}
						findElement(By.xpath("//table[contains(@class,'x-datepicker-inner x-focus x-datepicker-focus x-datepicker-default-focus')]//td[contains(@class,'x-datepicker-selected')]/parent::tr/child::td["+(i+2)+"]")).click();
						try {Thread.sleep(1000);}catch(InterruptedException ie) {}
	  ...
	  public void ClickOn_QuickActionOnMilestone()
	  {
		  try{Thread.sleep(6000);}catch(InterruptedException ie) {}
		    ...
		    actions.moveToElement(findElement(AddMilestoneOnMasterContainerInCVLocator.RightClick_OnMilestone())).contextClick().build().perform();
				...
				if(classAttributeValueOfQuickActionSideColumn.equals("x-menu-item-text x-menu-item-text-default x-menu-item-indent-right-arrow"))		//Class attribute is aParent means Quick Action icon is visible and click on it
				{
