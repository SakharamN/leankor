 ...
 //   explicitWaitVisibilityOf(findElement(CreateCardsOnDifferentMasterContainersOnCVLocator.ClickOnSchedulingMode()));
  //  findElement(CreateCardsOnDifferentMasterContainersOnCVLocator.ClickOnSchedulingMode()).click();
   ...
   // findElement(CreateCardsOnDifferentMasterContainersOnCVLocator.clickonEffortDrivenSchedulingMode()).click();
    ...
    for(int i=1; i<=3; i++)
    {
    	findElement(By.xpath("//div[@id='panel-1163-innerCt']/div/div["+i+"]")).click();
    }
...
//		findElement(CreateCardsOnDifferentMasterContainersOnCVLocator.uploadImageOnFilesInputFieldArea()).sendKeys(filePath+"\\ImageFolder/"+"download4.jpg");
	...
	//	explicitWaitVisibilityOf(findElement(CreateCardsOnDifferentMasterContainersOnCVLocator.ClickOnQuickAction()));
	//	action.moveToElement(findElement(CreateCardsOnDifferentMasterContainersOnCVLocator.ClickOnQuickAction())).build().perform();
			...
			if(classAttributeValueOfQuickActionSideColumn.equals("x-menu-item-link"))		//Class attribute is aParent means Quick Action icon is visible and click on it
			{
